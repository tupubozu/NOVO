...
import os, sys, re, zipfile, zlib, random, math
import pandas as pd
import numpy as num
...
def getDataMatrices(zipPath):
    try:
        with zipfile.ZipFile(zipPath,mode='r',compression=zipfile.ZIP_DEFLATED) as archive:
            test = archive.testzip()
            if test is None: 
                data = [(pd.read_csv(archive.open(entryInfo, mode='r')),f"{os.path.basename(archive.filename)}/{entryInfo.filename}") for entryInfo in archive.infolist() if os.path.splitext(entryInfo.filename)[1] == ".csv"]
            else:
                print(f"{archive.filename} contains bad file: {test} \nAborting...")
                data = [(pd.DataFrame({"Time":[0], "Channel": [0]}),'Error'),]
            
            archive.close()
            return data
    except Exception as ex:
        print(f"{zipPath}\n{ex}")
        return [(pd.DataFrame({"Time":[0], "Channel": [0]}),'Error'),]
...
def AvgPlot(dataMatrixCollection):
    timeVector = []
    limY = []
    limX = []
    
    cntChannels = 0
    for dataMatrix in dataMatrixCollection:
        dataVectors = [dataMatrix[column] for column in dataMatrix.columns]
        limY.append( 10 * int((math.ceil(max([max(dataVectors[i]) for i in range(1, len(dataVectors))])) + 10) / 10)), limY.append(10 * int((math.floor(min([min(dataVectors[i]) for i in range(1, len(dataVectors))])) - 10) / 10)), limX.append(max(dataVectors[0])), limX.append(min(dataVectors[0]))
        timeVector.extend(dataVectors[0])
        cntChannels = len(dataVectors) - 1
    
    timeVector.sort()
    channelCollection = [[] for i in range(cntChannels)]
    
    for channelIndex in range(len(channelCollection)):
        for val in timeVector:
            temp = 0
            
            for dataMatrix in dataMatrixCollection:
                dataVectors = [dataMatrix[column] for column in dataMatrix.columns]
                for i in range(len(dataVectors[0])- 1):
                    if dataVectors[0][i] <= val <= dataVectors[0][i + 1]: 
                        temp += (dataVectors[channelIndex + 1][i + 1] - dataVectors[channelIndex + 1][i])/(dataVectors[0][i + 1] - dataVectors[0][i]) * (val - dataVectors[0][i] ) + dataVectors[channelIndex + 1][i]
                        break
                    #elif val == dataVectors[0][i + 1]: 
                    #    temp += dataVectors[channelIndex + 1][i + 1]
                    #    break
            temp /= len(dataMatrixCollection)
            channelCollection[channelIndex].append(temp)            
    
    colour = ['r', 'b', 'g', 'm', 'y', 'c']
    [mplot.plot(timeVector, channelCollection[i], f'{colour[(i - 1) % len(colour)]}', label=f'Channel {i}', linewidth=0.5) for i in range(1, len(channelCollection))]

    mplot.yticks(num.arange(min(limY), max(limY), 10))
    mplot.xticks(num.arange(min(limX), max(limX), 5))
    mplot.xlabel('Time [ns]')
    mplot.ylabel('Amplitude [mV]')
    mplot.title(f'AvgPlot')
    mplot.legend()
    mplot.grid()
    mplot.show()
    mplot.close()
...
def Main(argv):
    print("NovoInspector v2.1.6\n")
    
    if len(argv) > 0:
        file_directories = [arg for arg in argv if zipfile.is_zipfile(arg) ]
        for arg in argv: 
    ...
    while True:
        try:
            ...
            if 0 <= dataSelector < len(dataCollection):
                ...
                while True:
                    command = input('Random plot[r], \"All\" plot[a], statistics[s], histograms[h] or exit[e]? \n\tCommand: ')
                    ...
                    if command == 'r':
                        ...
                        print('Plotting average/shadow waveform...')
                        AvgPlot(dataCollection[dataSelector][0])
                    elif command == 's':
